...
def main(cfg: FairseqConfig) -> None:
    ...
    for ende in ['encoder', 'decoder']:
        for _ly in range(0,6):
            ...
            for head in range(0,4): 
                ...
                p1 = _sd[_n1][128*head:128*(head+1), :]
                _n2 = f'{ende}.layers.{_ly}.self_attn.k_proj.weight'
                p2 = _sd[_n2][128*head:128*(head+1), :]

                qTk = torch.mm(p1.T, p2)
                U, S, Vh = torch.linalg.svd(qTk)
                # print(S)
                ...
                for i, ss in enumerate(S):
                    _acc += ss
                    if _acc > _alpha:
                        print(f"{_alpha*100:.0f}% | {ende} | {_ly} | {head} | {i}")
                        break
                        # print(f"{i:3d}| {_acc:.2f}")
    ...
    for ende in ['encoder', 'decoder']:
        for _ly in range(0,6):
            print("------------------------------------")
            for head in range(0,4): 
                _n1 = f'{ende}.layers.{_ly}.self_attn.v_proj.weight'
                p1 = _sd[_n1][128*head:128*(head+1), :]
                _n2 = f'{ende}.layers.{_ly}.self_attn.out_proj.weight'
                p2 = _sd[_n2][128*head:128*(head+1), :]
    ...
    for ende in ['encoder', 'decoder']:
        for _ly in range(0,6):
            ...
            for i, ss in enumerate(S):
                _acc += ss
                if _acc > _alpha:
                    print(f"{_alpha*100:.0f}% | {ende} | {_ly} | {i}")
                    break
                    # print(f"{i:3d}| {_acc:.2f}")
