...
"""
Finetuning the pruned model on one or across multiple GPUs.
"""
...
def main(cfg: FairseqConfig) -> None:
    ...
    # pass checkpoint path and reshaping model
    ...
    logger.info(
        "num. shared model params: %s (num. trained: %s)",
            sum(
    ...
    logger.info(
        "num. expert model params: %s (num. trained: %s)",
    ...
    logger.info(
        "training on %s devices (GPUs/TPUs)",
    ...
    while epoch_itr.next_epoch_idx <= max_epoch:
        ...
        if learning_rate <= cfg.optimization.stop_min_lr:
            logger.info(
                "stopping training because current learning rate (%s) is smaller "
                "than or equal to minimum learning rate "
                "(--stop-min-lr=%s)",
                learning_rate,
                cfg.optimization.stop_min_lr
            )
            break
        # train for one epoch
...
def should_stop_early(cfg: DictConfig, valid_loss: float) -> bool:
    ...
    if prev_best is None or is_better(valid_loss, prev_best):
        ...
        if should_stop_early.num_runs >= cfg.checkpoint.patience:
            logger.info(
                "early stop since valid performance hasn't improved for last %d runs",
...
def train(
    ...
    logger.info("end of epoch %s (average epoch stats below)", epoch_itr.epoch)
...
def validate_and_save(
    ...
    """Evaluate the model on the validation set(s) and return stop condition"""
    num_updates = trainer.get_num_updates()
    ...
    if num_updates >= max_update:
        should_stop = True
        logger.info(
            "Stopping training due to num_updates: %s >= max_update: %s",
            num_updates,
            max_update
        )
    ...
    if (
        ...
        should_stop = True
        logger.info(
            "Stopping training due to cumulative_training_time: %s > stop_time_hours: %s hour(s)",
            training_time_hours,
...
def get_training_stats(stats: Dict[str, Any]) -> Dict[str, Any]:
    """Aggregate training statistics from distributed workers."""
...
def get_valid_stats(
    ...
    """Aggregate validation statistics from distributed workers."""
...
def cli_main(
    ...
    if cfg.common.use_plasma_view:
        ...
        logger.info(
            "Started plasma server pid %d %s",
            server.server.pid,
            cfg.common.plasma_path,
        )
