...
from fairseq.modules import LayerNorm, SensiMixMultiheadAttention
from fairseq.modules.fairseq_dropout import FairseqDropout
from fairseq.modules.quant_noise import quant_noise
from torch import Tensor
from fairseq.models.sensimix import (
    SensiMixConfig,
)

# For SensiMix
from fairseq.modules.quantized_modules import *

class SensiMixEncoderLayerBase(nn.Module):
    """Encoder layer block.
    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def __init__(self, cfg, _idx, return_fc=False):
        super().__init__()
        self.cfg = cfg
        ...
        # _idx: layer_num (0 base)

        if (cfg.encoder_layers - _idx) < (cfg.encoder_1_layers+ 1):
            _bits = 1
        else:
            _bits = 8
    ...
    def build_fc1(self, input_dim, output_dim, q_noise, qn_block_size, _bits=32):
        assert _bits in [1, 8, 32]
        if _bits == 1:
            return quant_noise(
                b_Linear(input_dim, output_dim), p=q_noise, block_size=qn_block_size
            )
        elif _bits == 8:
            return quant_noise(
                q_Linear(input_dim, output_dim), p=q_noise, block_size=qn_block_size
            )
        else:
            return quant_noise(
    ...
    def build_fc2(self, input_dim, output_dim, q_noise, qn_block_size, _bits=32):
        assert _bits in [1, 8, 32]
        if _bits == 1:
            return quant_noise(
                b_Linear(input_dim, output_dim), p=q_noise, block_size=qn_block_size
    ...
    def build_self_attention(self, embed_dim, cfg):
        return SensiMixMultiheadAttention(
            embed_dim,
...
class SensiMixEncoderLayer(SensiMixEncoderLayerBase):
    def __init__(self, args):
        super().__init__(SensiMixConfig.from_namespace(args))
        self.args = args

    def build_self_attention(self, embed_dim, args):
        return super().build_self_attention(
            embed_dim, SensiMixConfig.from_namespace(args)
        )


class SensiMixDecoderLayerBase(nn.Module):
    """Decoder layer block.
    ...
    *cfg.decoder.normalize_before* to ``True``.
    ...
    def __init__(
        self, cfg, _idx, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False
    ):
        super().__init__()
        ...
        # Number of bits for fully connected layers
        if (cfg.decoder_layers - _idx) < (cfg.decoder_1_layers+ 1):
            _bits = 1
        else:
            _bits = 8
    ...
    def build_fc1(self, input_dim, output_dim, q_noise, qn_block_size, _bits=32):
        assert _bits in [1, 8, 32]
        if _bits == 1:
            return quant_noise(b_Linear(input_dim, output_dim), q_noise, qn_block_size)
        elif _bits == 8:
            return quant_noise(q_Linear(input_dim, output_dim), q_noise, qn_block_size)
        else:
...
# backward compatible with the legacy argparse format
class SensiMixDecoderLayer(SensiMixDecoderLayerBase):
    def __init__(
        self, args, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False
    ):
        super().__init__(
            SensiMixConfig.from_namespace(args),
