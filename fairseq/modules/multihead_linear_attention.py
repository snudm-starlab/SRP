...
class MultiheadLinearAttention(nn.Module):
    ...
    def __init__(
        ...
        if shared_compress_layer is None:
            self.compressed_kdim = self.kdim // compressed
            self.compressed_vdim = self.vdim // compressed
            self.E = nn.Linear(self.kdim, self.compressed_kdim, bias=False)
            if not shared_kv_compressed:
                self.F = nn.Linear(self.vdim, self.compressed_vdim, bias=False)
            else:
                self.F = self.E
            self.layerwise_sharing = False
        else:
            self.E = shared_compressed_layer
            if not shared_kv_compressed:
                self.F = shared_compressed_lyer
            else:
                self.F = self.E
        ...
        self.E = quant_noise(
            nn.Linear(self.kdim, self.embed_dim, bias=bias), q_noise, qn_block_size
            )

        self.F = quant_noise(
            nn.Linear(self.kdim, self.vdim, bias=bias), q_noise, qn_block_size
            )
        """
    ...
    def reset_parameters(self):
        if self.qkv_same_dim:
            ...
            if (not self.layerwise_sharing):
                # otherwise, E and F are already initialized
                nn.init.xavier_uniform_(self.E.weight, gain=1 / math.sqrt(2))
                if not self.shared_kv_compressed:
                    nn.init.xavier_uniform_(self.F.weight, gain=1/ math.sqrt(2))
        else:
            ...
            if (not self.layerwise_sharing):
                ...
                if not self.shared_kv_compressed:
                    nn.init.xavier_uniform_(self.F.weight, gain=1.)
    ...
    def forward(
        ...
        if (
            ...
            """ TODO: Check whether this part is runned or not"""
            ...
            return F.multi_head_attention_forward(
                ...
                k_proj_weight=torch.mm(self.E.weight, self.k_proj.weight),
                v_proj_weight=torch.mm(self.F.weight, self.v_proj.weight),
            )
        ...
        if self.self_attention:
            q = self.q_proj(query)
            k = self.E(self.k_proj(query))
            v = self.F(self.v_proj(query))
            ...
            if key is None:
                ...
                k = self.E(self.k_proj(key))
                v = self.F(self.v_proj(key))

        else:
            assert key is not None and value is not None
            q = self.q_proj(query)
            k = self.E(self.k_proj(key))
            v = self.F(self.v_proj(value))
        q *= self.scaling        
