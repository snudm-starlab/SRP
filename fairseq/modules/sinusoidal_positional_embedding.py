...
# For SensiMix
class qSinusoidalPositionalEmbedding(nn.Module):
    """This module produces sinusoidal positional embeddings of any length.
    ...
    def __init__(self, embedding_dim, padding_idx, init_size=1024):
        ...
        self.embedding_dim = embedding_dim
        self.padding_idx = padding_idx if padding_idx is not None else 0
        self.weights = qSinusoidalPositionalEmbedding.get_embedding(
            init_size, embedding_dim, padding_idx
        )
        self.onnx_trace = False
        self.register_buffer("_float_tensor", torch.FloatTensor(1))
    ...
    def forward(
        ...
        a = float(torch.min(self.weights.data))
        b = float(torch.max(self.weights.data))
        ...
        self.weights.data = quantization(self.weights.data, 8)
