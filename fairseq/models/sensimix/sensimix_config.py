...
class EncDecBaseConfig(FairseqDataclass):
    ...
    # args for "Reducing SensiMix Depth on Demand with Structured Dropout" (Fan et al., 2019)
...
@dataclass
class SensiMixConfig(FairseqDataclass):
    ...
    # args for "Cross+Self-Attention for SensiMix Models" (Peitz et al., 2019)
    ...
    # copied from sensimix_lm but expected in sensimix_decoder:
    ...
    # for subclasses of SensiMix(Legacy) that depend on read/write on
    # the flat namespace.
    ...
    def from_namespace(cls, args):
        ...
        if not isinstance(args, cls):
            ...
            # for now.
            for fld in fields(cls):
                # concretelly, the sensimix_config know what sub-dc it has, so we go through all the dc fields
